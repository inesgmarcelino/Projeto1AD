...
BASE_URL = 'https://api.spotify.com/v1/'  # API
...
def searchU():
    ...
    if not request.data:
        ...
        if id_user in ids:
            query = conn.execute('SELECT * FROM utilizadores WHERE id = ' + str(id_user)).fetchall()
            lines = []
            for i in range(len(query)):
                id_user = query[i][0]
                nome = query[i][1]
...
def deleteU():
    ...
    if not request.data:
        ...
        if id_user in ids:
            query = conn.execute('DELETE FROM listas_albuns WHERE id_user = ' + str(id_user))
            conn.commit()
            query = conn.execute('DELETE FROM utilizadores WHERE id = ' + str(id_user))
            conn.commit()
...
def createU():
    ...
    nome = data['nome']
    senha = data['pass']
    query = conn.execute("INSERT INTO utilizadores (id, nome, senha) VALUES (" + str(id)+",'"+nome+"','"+senha+"')")
    conn.commit()
    r = make_response('Novo utilizador adicionado! Nome: %s, ID: %s' % (nome, id), 201)
    conn.close()
    return r
...
def updateU():
    ...
    if id_user in ids:
        senha = data['pass']
        query = conn.execute("UPDATE utilizadores SET senha = '"+senha+"' WHERE id = "+str(id_user))
        conn.commit()
    ...
    if not request.data:
        ...
        if len(query) != 0:
            ...
            for i in range(len(query)):
                ...
                lines.append('ID: %s\nID Spotify: %s\nNome: %s' % (id_artista, id_spotify, nome))
        ...
        if id_artista in ids:
            query = conn.execute('SELECT * FROM artistas WHERE id = ' + str(id_artista)).fetchall()
            if len(query) != 0:
                lines = []
                for i in range(len(query)):
                    id_artista = query[i][0]
...
def deleteA():
    ...
    if not request.data:
        ...
        id_artista = data['id_artista']
        if id_artista in ids:
            ids_album = conn.execute('SELECT albuns.id FROM albuns WHERE albuns.id_artista = ' + str(id_artista)).fetchall()
            for i in range(len(ids_album)):
                query = conn.execute('DELETE FROM listas_albuns WHERE id_album = ' + str(ids_album[i][0]))
                conn.commit()
            query = conn.execute('DELETE FROM albuns WHERE id_artista =' + str(id_artista))
            conn.commit()
            query = conn.execute('DELETE FROM artistas WHERE id = ' + str(id_artista))
            conn.commit()
...
def createA():
    ...
    if artista not in artistas:
        id = conn.execute('SELECT COUNT(id) FROM artistas').fetchall()[
            0][0] + 1
        r = spotify.get(BASE_URL + 'artists/' + artista, headers=None)
        api = r.json()
        nome = api['name']
        query = conn.execute("INSERT INTO artistas (id, id_spotify, nome) VALUES ("+str(id)+",'"+artista+"','"+nome+"')")
        conn.commit()
    ...
    if len(query) != 0:
        if not request.data:
            ...
            for i in range(len(query)):
                ...
                id_spotify = query[i][1]
                nomeAlbum = query[i][2]
                nomeArtista = conn.execute('SELECT nome FROM artistas WHERE id = ' + str(query[i][3])).fetchall()[0][0]

                lines.append('ID: %s\nID Spotify: %s\nNome: %s\nArtista: %s' % (id_album, id_spotify, nomeAlbum, nomeArtista))
            ...
            if 'id_album' in data.keys():
                ...
                if id_album in ids:
                    query = conn.execute('SELECT * FROM albuns WHERE id = ' + str(id_album)).fetchall()
                    if len(query) != 0:
                        lines = []
                        for i in range(len(query)):
                            id_album = query[i][0]
                ...
                id_artista = data['id_artista']
                if id_artista in ids:
                    query = conn.execute('SELECT * FROM albuns WHERE id_artista = ' + str(id_artista)).fetchall()
                    if len(query) != 0:
                        lines = []
                        for i in range(len(query)):
                            id_user = query[i][0]
                            ...
                            lines.append('ID: %s\nID Spotify: %s\nNome: %s\nArtista: %s' % (id_user, id_spotify, nomeAlbum, nomeArtista))
                        res = '\n--- ---\n'.join(tuple(lines))
                        code = 200
                ...
                query = conn.execute('SELECT DISTINCT id_user FROM listas_albuns').fetchall()
                ids = []
                for i in range(len(query)):
                    ids.append(query[i][0])
                ...
                id_user = data['id_user']
                if id_user in ids:
                    query = conn.execute('SELECT albuns.* FROM listas_albuns l, albuns WHERE l.id_user = %s AND albuns.id = l.id_album' % id_user).fetchall()
                    if len(query) != 0:
                        lines = []
                        for i in range(len(query)):
                            id_user = query[i][0]
                ...
                query = conn.execute('SELECT DISTINCT avaliacoes.sigla FROM avaliacoes, listas_albuns WHERE listas_albuns.id_avaliacao = avaliacoes.id').fetchall()
                avals = []
                for i in range(len(query)):
    ...
    if len(query) != 0:
        if not request.data:
            ...
            if 'id_album' in data.keys():
                ...
                if id_album in ids:
                    query = conn.execute('DELETE FROM listas_albuns WHERE id_album = ' + str(id_album))
                    conn.commit()
                    query = conn.execute('DELETE FROM albuns WHERE id = ' + str(id_album))
                    conn.commit()
                ...
                if id_user in ids:
                    ids_album = conn.execute('SELECT listas_albuns.id_album FROM listas_albuns WHERE listas_albuns.id_user = ' + str(id_user)).fetchall()
                    for i in range(len(ids_album)):
                        query = conn.execute('DELETE FROM listas_albuns WHERE id_album = ' + str(ids_album[i][0]))
                        conn.commit()
                        query = conn.execute('DELETE FROM albuns WHERE id =' + str(ids_album[i][0]))
                        conn.commit()
                ...
                aval = data['aval']
                if aval in avals:
                    id_aval = conn.execute('SELECT id FROM avaliacoes WHERE sigla = ' + aval)
                    ids_album = conn.execute('SELECT listas_albuns.id_album FROM listas_albuns WHERE listas_albuns.id_avaliacao = ' + str(id_aval)).fetchall()
                    for i in range(len(ids_album)):
                        query = conn.execute('DELETE FROM listas_albuns WHERE id_album = ' + str(ids_album[i][0]))
                        conn.commit()
                        query = conn.execute('DELETE FROM albuns WHERE id =' + str(ids_album[i][0]))
...
def createAB():
    ...
    if len(data) == 1:
        ...
        if album not in albuns:
            r = spotify.get(BASE_URL + 'albums/' + album, headers=None)
            api = r.json()
            nome = api['name']
            artista = api['artists'][0]['name']
            id = conn.execute('SELECT COUNT(id) FROM albuns').fetchall()[0][0] + 1
            ...
            if artista in artistas:
                id_artista = conn.execute("SELECT id FROM artistas WHERE nome = '" + artista + "'").fetchall()[0][0]
                query = conn.execute('INSERT INTO albuns VALUES (?,?,?,?)', (id, album, nome, id_artista))
                conn.commit()
                res = 'Novo álbum adicionado! ID: %s, Nome: %s, ID Spotify: %s, Artista: %s' % (id, nome, album, id_artista)
                code = 201
        ...
        query = conn.execute('SELECT * FROM listas_albuns l WHERE l.id_user = %s AND l.id_album = %s' % (id_user, id_album)).fetchall()

        if len(query) == 0:
            aval = data['aval']
            id_aval = conn.execute("SELECT id FROM avaliacoes WHERE  sigla = '" + aval + "'").fetchall()[0][0]
            query = conn.execute('INSERT INTO listas_albuns VALUES (?, ?, ?)', (id_user, id_album, id_aval))
            conn.commit()
            res = 'Nova avaliação foi adicionada! ID: %s, Álbum: %s, Avaliação: %s' % (id_user, id_album, aval)
            code = 201
        else:
...
def updateAB():
    ...
    if id_album in albuns:
        ...
        if aval in avals:
            query3 = conn.execute('SELECT DISTINCT id_user FROM listas_albuns').fetchall()
            users = []
            ...
            if id_user in users:
                id_aval = conn.execute("SELECT id FROM avaliacoes WHERE  sigla = '" + aval + "'").fetchall()[0][0]
                query = conn.execute('UPDATE listas_albuns SET id_avaliacao = %s WHERE id_user = %s AND id_album = %s' % (id_aval, id_user, id_album))
                conn.commit()
                res = 'A avaliação do utilizador %s ao álbum %s foi alterada para %s.' % (id_user, id_album, aval)
                code = 201
...
if __name__ == '__main__':
    ...
    context.load_verify_locations(cafile='grupo25/certs/root.pem')
    context.load_cert_chain(certfile='grupo25/certs/serv.crt', keyfile='grupo25/server/serv.key')
